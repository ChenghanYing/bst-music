...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
    ...
    if (key < root -> key) return BST_search(root -> left, bar, index);
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     * Implement this function
     ****/
  double key = bar * 10 + index;
  BST_Node *tmp;
  if (root==NULL) return NULL;
  if (root -> key < key) root -> right = BST_delete(root -> right, bar, index);
  else if (root -> key > key) root -> left = BST_delete(root -> left, bar, index);
  else
  {
...
void find_max_and_min(BST_Node *root, int *min, int *max)
{
    if (root == NULL) return;
    if (root -> bar < *min) *min = root -> bar;
    if (root -> bar > *max) *max = root -> bar;
    find_max_and_min(root -> left, min, max);
    find_max_and_min(root -> right, min, max);
}

BST_Node *reverse(BST_Node *old_root, BST_Node *new_root, int min, int max)
{
    BST_Node * new_node;
    int new_bar;
    double new_index;
    if (old_root == NULL) return new_root;
    new_bar = max + min - old_root -> bar;
    new_index = 1.0 - old_root -> index;
    new_node = newBST_Node(old_root -> freq, new_bar, new_index);
    new_root = BST_insert(new_root, new_node);
    new_root = reverse(old_root -> left, new_root, min, max);
    new_root = reverse(old_root -> right, new_root, min, max);
    return new_root;
}
BST_Node *reverseSong(BST_Node *root)
{
    ...
    min = root -> bar;
    max = root -> bar;
    find_max_and_min(root, &min, &max);
    new_root = NULL;
    new_root = reverse(root, new_root, min, max);
    delete_BST(root);
    return new_root;
}
...
BST_Node *harmonize_helper(BST_Node *root, BST_Node *curr, int semitones, double time_shift)
{
    int p = -1;
    int new_p;
    double new_index;
    ...
    left_temp = curr->left;
    right_temp = curr->right;

    
    for (int i = 0; i < 100; i++)
    {
        if (note_freq[i] == curr->freq) p = i;
    }

    if (p != -1)
    {
        new_p = p + semitones;
        new_index = curr->index + time_shift;

        if (new_p >= 0 && new_p < 100 && new_index >= 0 && new_index < 1)
        {
            new_freq = note_freq[new_p];
            new_node = newBST_Node(new_freq, curr->bar, new_index);
            root = BST_insert(root, new_node);
        }
    }
    root = harmonize_helper(root, left_temp, semitones, time_shift);
    root = harmonize_helper(root, right_temp, semitones, time_shift);
    return root;
}
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     * Implement this function
     ****/
    return harmonize_helper(root, root, semitones, time_shift);
}
